...
public class RouteMap {
	...
	public int addLocation(String name, double latitude, double longitude, int id){
		if(locations.containsKey(id)){
			...
			KpsModel.println(String.format("Added location %s (%.2f, %.2f) with id: %d", name, latitude, longitude, id));
			return id;
		}
	}
	...
	public int addSegment(int id, int originId, int destinationId){
		...
		if(!locations.containsKey(originId)){
			KpsModel.println(String.format("Error: Cannot add segment as origin location with id %d does not exist.", originId));
			return -1;
		}
		if(getSegmentIdFrom(originId, destinationId) != -1){
			KpsModel.println(String.format("Error: Cannot add segment as origin location with id %d already has segment to destination with id %d.",  originId, destinationId));
			return -1;
		}
		if(!locations.containsKey(destinationId)){
			KpsModel.println(String.format("Error: Cannot add segment as destination location with id %d does not exist.", destinationId));
			return -1;
		}
		...
		KpsModel.println(String.format("Added segment from %s to %s with id: %d", locations.get(originId).getName(), locations.get(destinationId).getName(), id));
		return id;
	}
